        ...
        stage('Sync S3'){
           if (params.SYNC_TO_S3 == false) {
            ...
            dir(WORKSPACE){
                ...
                runningPod=$(kubectl get po -o=name -l app=${PROJECT_NAME}-g --sort-by=".metadata.name" -n mtw-${BUILD_ENV} --field-selector=status.phase==Running -o 'jsonpath={.items[0].metadata.name}' | sort -r)
                echo $runningPod
