...
export default function Modals() {
  ...
  function touchMoveEventHandler(type: 'add' | 'remove', isForceRun = false) {
  ...
  useEffect(() => {
    router.beforePopState(({ as, options }) => {
      ...
      // 모달 띄워진 상태에서 메인으로 이동시 스크롤 활성화
      if (PAGES_WITHOUT_BOTTOM_NAVI.includes(router.basePath + router.pathname)) {
        enableScroll();
      }
  ...
  useEffect(() => {
    if (openedModals.length > 0 || openedOverlayModals.length > 0) {
      ...
      if (isAndroid()) {
        const exceptHistoryPush =
          _.findIndex(openedModals.concat(openedOverlayModals), (modal) => modal.Component === BottomSheetLogin) > -1;
        if (!exceptHistoryPush) {
    ...
    if (openedModals.length > 0 || openedOverlayModals.length > 0) {
      touchMoveEventHandler('add', doNotBottomNaviControl);
      if (doNotBottomNaviControl === false) window.$tw.bottomNav$api.hide();
    } else {
      touchMoveEventHandler('remove', doNotBottomNaviControl);
      if (doNotBottomNaviControl === false) window.$tw.bottomNav$api.show();
    }
  }, [openedModals, openedOverlayModals, isDocumentHidden]);
