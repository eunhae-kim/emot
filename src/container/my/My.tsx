...
import TMembership from './TMembership';
...
export default function () {
  ...
  const 무선요금 = useMemo(() => {
    if (billsScreenType === '무선_통합청구_일반회선') {
      return lastMonthBillsDisplayData?.이용요금;
    }
    return lastMonthBillsDisplayData?.청구요금;
  }, [billsScreenType, JSON.stringify(lastMonthBillsDisplayData)]);

  const 유선요금 = useMemo(() => {
    if (billsScreenType === '무선_통합청구_일반회선') {
      return wireDisplayData?.이용요금;
    }
    return wireDisplayData?.청구요금;
  }, [billsScreenType, wireDisplayData]);
  ...
  useEffect(() => {
    ...
    const _use요금 = _billsScreenType !== '준회원';
    setUse요금(_use요금);
      ...
      if (_billsScreenType === '무선_PPS') {
        ...
        paidApi({
          setter: (apiResp) => {
            ...
            // B청구 회선은 통합청구 분류 안함
            if (wireDisplayData.통합청구사용 && _myDisplayData.선택회선.svcInfo.actCoClCd !== 'B') {
              setBillsScreenType(
              ...
              <div className="my-box">
                ...
                {!billsInquiryException && !ppsDisplayData?.사용가능금액 && ppsDisplayData?.사용가능데이터 && (
                  <>
                    ...
                    <XtrAw appEid="CMMA_A24-67" webEid="MWMA_A24-71" xtrView xtrClick>
                ...
                {ppsDisplayData && ppsDisplayData.사용가능데이터 && ppsDisplayData.사용가능금액 && (
                  <div className="fee-list">
                    <XtrAw appEid="CMMA_A24-69" webEid="MWMA_A24-73" xtrView xtrClick>
                      <MyItem
                        icon={setMyItem.args.icon}
                        title="사용할 수 있는 데이터"
            ...
            {use요금 &&
              (billsScreenType === '유선_일반' ||
                ...
                <div className="my-box">
                  ...
                  {!billsInquiryException &&
                    wireDisplayData &&
                    (billsScreenType === '유선_통합청구_일반회선' || billsScreenType === '유선_SK브로드밴드') && (
                  ...
                  {wireDisplayData?.이용요금 && (
                    <>
                      <XtrAw appEid="CMMA_A24-17" webEid="MWMA_A24-49" xtrView xtrClick>
                        <MyVolume over1={유선요금} overUnit1="원" link="/myt-fare/submain" />
                      </XtrAw>
