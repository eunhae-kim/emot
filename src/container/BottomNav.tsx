...
let SKIP_REMOVE_BODY_CLASS = false;
...
export type LDSP_TYPE = 'LOCAL' | 'STG_LOCAL' | 'GREEN_LOCAL' | 'DEV' | 'PRJ_DEV' | 'STG' | 'GREEN' | 'BLUE';
...
const generateLandingInfo = (tworldUrl: string, lang: Lang) => {
  return {
    login: loginUrl(tworldUrl, lang, true),
    ...
    korTworld: v6TworldUrl(tworldUrl || window.location.origin, 'KO'),
  };
};
...
const getMenuInfoMap = (tworldUrl: string, shopUrl: string, lang: Lang, ldsp: LDSP_TYPE) => {
  ...
  let baseUrl = tworldUrl;
  // V6페이지로 상대 경로로 이동 시에 props 변경 잘 인식 되지 않는 케이스 있어서 절대 경로로 변경
  if (!baseUrl) {
  ...
  if (lang === 'KO') {
    menuInfoMap = {
      MY: {
        label: 'MY',
        url: `${baseUrl}/v6/my?returnUrl=${window.location.href}`,
      },
      HOME: {
        label: '홈',
        url: `${baseUrl}/v6/main`,
    ...
    menuInfoMap = {
      MY: {
        label: 'MY',
        url: `${baseUrl}/v6/en/my?returnUrl=${window.location.href}`,
      },
      HOME: {
        label: 'HOME',
        url: `${baseUrl}/en/main/home`,
      },
      MENU: {
        label: 'MENU',
        url: `${baseUrl}/v6/en/menu`,
      },
    };
  }
...
const generateDefaultProps = (allProps: BottomNavContainerProps) => {
  ...
  const tworldUrl = tworldUrlParam;
...
const removeBodyClass = () => {
  // v6 => v6 history api로 이동 시에 이전 페이지 클래스 남는 현상 수정
  if (document?.body?.classList?.remove) {
    document.body.classList.remove('g-bottom');
    document.body.classList.remove('g-bottom-sheet');
  }
};
...
const hideBottomNav = (showNavClass) => {
  ...
  if (!SKIP_REMOVE_BODY_CLASS) {
    removeBodyClass();
  }

  SKIP_REMOVE_BODY_CLASS = false;
...
export default function BottomNav(allProps: BottomNavContainerProps) {
  ...
  if (!isServer()) {
    ...
    window.$tw.bottomNav$api = {
      ...
      hide: (shouldForce?: boolean, skipRemoveBodyClass?: boolean) => {
        if (isVisible) {
          SKIP_REMOVE_BODY_CLASS = skipRemoveBodyClass;
        }
