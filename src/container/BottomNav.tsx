...
import { callBridgeApi, getBillsScreenType, isLoggedIn, isServer } from '../common/utils';
...
export type LDSP_TYPE = 'LOCAL' | 'DEV' | 'STG' | 'GREEN' | 'BLUE';

export type BottomNavContainerProps = {
  ...
  // only applicable when isTworld === false
  isLoggedIn?: boolean;
  loginUrl?: string;
...
const generateDefaultProps = (allProps: BottomNavContainerProps) => {
  ...
  let bottomSheetMode: BottomSheetMode;
...
export default function BottomNav(allProps: BottomNavContainerProps) {
  ...
  let myInfo, setMyInfo, myInfoLoaded, setMyInfoLoaded, compProps, setCompProps;
  if (props.isStandalone) {
    [myInfo, setMyInfo] = useState({});
  ...
  useEffect(() => {
    ...
    let lineIdText = '';
    let showBottomSheet = props.showBottomSheet;

    let myDisplayData = myApiRespToDisplayData(myInfo);
    let bottomSheetMode: BottomSheetMode = 'NONE';

    let loginStatus: boolean;
    if (isTworld) {
      loginStatus = isLoggedIn(myInfo);
    } else {
