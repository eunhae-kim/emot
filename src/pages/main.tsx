...
import { exitApp, getDeviceSegName, getOSInfo, getScmSegName, isAndroid, isApp } from '../js/commonUtil';
...
import { callBridgeApi, isCheckFromWidgetParam } from '../common/utils';
...
import ADotBaseball from '../container/main/ADotBaseball';
...
export default function Main({ ...props }) {
  const { confirm, termsAgreement, emergency, newApp, homePopupBanner } = useModal();
  ...
  const [cardListUpdatedAt, setCardListUpdatedAt] = useState<string>('');
  ...
  const [isShowHomePopupBanner, setIsShowHomePopupBanner] = useState<boolean>(false);
  ...
  useEffect(() => {
    if (showTerms) {
      // 1. my로 redirect 되는 상황이면 표출하지 않음(위젯에서 접근되었을때)
      // 2. 홈 팝업 배너 표출되는 상황이면 표출하지 않음
      if (isCheckFromWidgetParam() || isShowHomePopupBanner) return;
      console.log('show termsAgreement : ', isShowHomePopupBanner);
      termsAgreement.show();
    }
  }, [showTerms]);

  // 긴급공지 모달
  async function getEmergencyNotice() {
  ...
  // 홈 팝업 배너 표출 여부 확인 (해당 배너는 메인 접근시 마다 표출)
  async function checkHomePopupBanner() {
    try {
      const response = await getHomePopupBanner();
      if (response === null || response.bannerList.length === 0) return;
      // 여러개의 배너가 있을때는 게시 시작일이 가장 큰 값을 선택
      const bannerInfo = response.bannerList.reduce((max: any, obj: any) => {
        return obj.expsStaDtm > max.expsStaDtm ? obj : max;
      }, response.bannerList[0]);

      const localStorageKey = `homePopupBanner${bannerInfo.bnnrSeq}`;
      if (localStorage.getItem(localStorageKey)) {
        const hiddenDate = new Date(localStorage.getItem(localStorageKey));
        const today = new Date();
        if (hiddenDate.getTime() >= today.getTime()) {
          return;
        }
      }
      // 홈팝업배너 관련 로컬 스토리지 초기화(신규 배너를 즉시 노출하기 위해)
      Object.keys(localStorage)
        .filter((x) => x.startsWith('homePopupBanner'))
        .forEach((x) => localStorage.removeItem(x));

      setIsShowHomePopupBanner(true);
      homePopupBanner.show({
        isOpen: true,
        bannerInfo,
      });
    } catch (e) {
      console.error('getHomePopupBanner error');
    }
  }
  ...
  useEffect(() => {
    // 위젯에서 접근되었는지 확인(위젯 접근시 바로 My 페이지 띄워야 하기 때문에 모달 표출하면 안됨)
    if (!isCheckFromWidgetParam()) {
    ...
    addToOnBackHandlers({ handlerName: 'exitApp', handlerFunction: exitAppFunction });

    return () => {
      removeFromOnBackHandlers('exitApp');
    };
  }, []);
  ...
  useEffect(() => {
    ...
    if (isCheckFromWidgetParam() && bottomNavCompProps.myButtonAction === 'MY') {
  ...
  function mainInitModal() {
    (async () => {
      await getEmergencyNotice(); // 긴급공지
      await checkNewAppYn(); // 소문자앱 업데이트
      await checkHomePopupBanner(); // 홈팝업배너 (홈팝업배너 노출시 선택약관 모달은 미노출)
    })();
  }
  ...
  function alignCards(mlsData) {
    ...
    if (response) {
      const respUpdatedAt = mlsData.updatedAt?.substring(0, 8);
      setCardListUpdatedAt(respUpdatedAt);
      Object.keys(mlsData.cardItems).forEach((k) => {
        const v = mlsData.cardItems[k];
        // 일단은 adot 프로야구카드 한정으로 data.displayYn으로 노출 여부 선택
        if (k === 'adot_001' && v && v.data && v.data.displayYn === 'N') {
          const idx = response.indexOf(k);
          if (idx > -1) {
      ...
      {cardList.map((card) => {
        switch (card) {
          ...
          case 'adot_001':
            return (
              <div key="adot_001">
                <ADotBaseball targetDate={cardListUpdatedAt || 'latest'} />
              </div>
            );
          default:
            return null;
