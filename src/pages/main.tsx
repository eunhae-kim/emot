...
import { getDeviceSegName, getScmSegName, exitApp, isApp, getOSInfo } from '../js/commonUtil';
...
import { MainTtimeSlide } from '../components/Main/MainTtimeSlide';
import { Default as MainTtimeStoryProps } from '../components/Main/MainTtimeSlide.stories';
...
export default function Main() {
  ...
  const addToOnBackHandlers = appContext.addToOnBackHandlers;
  const getMccCode = appContext.getMccCode;
  const [mccCode] = appContext.mccCode;
  const [, setIsMenuEditYn] = appContext.isMenuEditYn;

  const [membershipType, setMembershipType] = useState<'01' | '02'>('01');
  const [svcMgmtNum, setSvcMgmtNum] = useState<string>(null);
  const [menuEditYn, setMenuEditYn] = useState(false);
  const menuEditYnRef = useRef<boolean>(false);
      ...
      switch (entry.target.id) {
        ...
        case 'vcolor_001': {
          if (cardItems['vcolor_001']) {
            const data = cardItems['vcolor_001'].data;
            if (!data) return;
            callConversion(svcMgmtNum, data.channelId, data.processId, data.id, 'impression');
            break;
          }
  ...
  useEffect(() => {
    menuEditYnRef.current = menuEditYn;
    setIsMenuEditYn(menuEditYn);
  }, [menuEditYn]);
  ...
  useEffect(() => {
    ...
    addToOnBackHandlers(() => {
      ...
      if (modalIsVisibleRef.current || menuEditYnRef.current) return;
      ...
      if (myInfo) {
        ...
        if (!loginYn) {
          ...
          if (myInfo.svcInfo?.isAdult && myInfo.svcInfo?.mbrChlId) {
            ...
            if (isApp() && sessionStorage.getItem('alreadyReadMCC') !== 'y') {
  ...
  return (
    <Layout addClass={!menuEditYn && 'main'}>
      <html lang="ko" />
      <Head>
        <title>홈 &lt; Tworld</title>
      </Head>
      {menuEditYn && <FullMenuEdited setMenuEditYn={setMenuEditYn} />}
      ...
      {/* 메뉴 바로가기 */}
      <ShortCutMenu setMenuEditYn={setMenuEditYn} />

      {/* 신규 t-time */}
      <MainTtimeSlide tTimeSlide={MainTtimeStoryProps.args.tTimeSlide} />
