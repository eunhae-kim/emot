...
// baseUrl이 명시적으로 지정된 경우(m.tworld 도메인이 아님) 타겟에 도메인 포함 절대 경로 지정
export const v5Target = (useFullUrl: boolean) => {
  if (useFullUrl) {
    return location.origin + getPathname();
  }
  return getPathname();
};
export const loginUrl = (baseUrl: string, lang: Lang, includeParam = false) => {
  return `${typeof baseUrl === 'string' ? baseUrl : ''}/${lang === 'EN' ? 'en/' : ''}common/tid/login?target=${
    v5Target(!!baseUrl) + (includeParam ? getSearchParam() : '')
  }&app=${isApp() ? 'Y' : 'N'}`;
};
/*
...
export const signUpUrl = (baseUrl: string, lang: Lang) => {
  ...
  if (lang === 'EN') {
    return `${baseUrl}/en/common/tid/signup-foreigner?target=${v5Target(!!baseUrl)}`;
  }
...
export const quickLoginUrl = (baseUrl: string, lang: Lang) => {
  if (isIos())
    return `${typeof baseUrl === 'string' ? baseUrl : ''}/${
      lang === 'EN' ? 'en/' : ''
    }common/member/slogin/ios?target=${v5Target(!!baseUrl)}`;
  if (isAndroid())
    return `${typeof baseUrl === 'string' ? baseUrl : ''}/${
      lang === 'EN' ? 'en/' : ''
    }common/member/slogin/aos?target=${v5Target(!!baseUrl)}`;
};
