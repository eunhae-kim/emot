import { BillsScreenType, CardProps, InquiryExcpetion, InquiryExcpetionType, Lang } from './types';
...
export const secToDisplayText = (sec: number, lang: Lang) => {
  const t = secToValueAndUnitArr(sec, lang);
  const space = lang === 'EN' ? '\u00A0' : '';
  return t.map((t) => `${t.v}${t.unit}`).join(space);
};
...
export const formatYYYYMDHHmm = (YYYYMMDDHHmm: string) => {
  ...
  if (YYYYMMDDHHmm.length === 12) {
    return (
      YYYYMMDDHHmm.substr(0, 4) +
      '. ' +
      String(parseInt(YYYYMMDDHHmm.substr(4, 2), 10)) +
      '. ' +
      String(parseInt(YYYYMMDDHHmm.substr(6, 2), 10)) +
      ' ' +
      YYYYMMDDHHmm.substr(8, 2) +
      ':' +
      YYYYMMDDHHmm.substr(10, 2)
...
export const getBillsScreenType = (myDisplayData: any) => {
  ...
  if (!myDisplayData.선택회선) {
    ...
    if (myDisplayData.선택회선.svcInfo.coClCd === 'B') {
...
export const myRedirect = (url: string) => {
  if (isApp()) {
    url = toFullUrl(url);
...
export const makeBannerList = (bannerResp, maxCount = 10) => {
  ...
  const bannerArray: Array<CardProps> = [];
  ...
  if (bannerListFromAdmin) {
    bannerListFromAdmin.map((banner) => {
      const card = {
