      ...
      {showListHeader && (
        <div className="list-head">
          ...
          <div className="select-option-box">
            {orderByText && (
              <XtrAw appEid="CMMA_A23_B121_C1419-9" webEid="CMMA_A23_B121_C1419-9" xtrClick xtrView>
      ...
      <ul className="story-list">
        {storyList?.map((item: any, index: number) => (
          ...
          <React.Fragment key={`${item.id}_${storyList.length}`}>
            ...
            {/* 리워드 띠배너 */}
            {isShowBanner && index === Math.floor(storyList.length / 2) - 1 && <TtimeBanner bannerType="mainList" />}
          </React.Fragment>
        ))}
      </ul>
    </article>
