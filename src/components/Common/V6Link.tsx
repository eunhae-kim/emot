...
export default forwardRef(({ href, newWindow, myToMainWebView, billYn, ...otherProps }: V6LinkProps, ref) => {
  ...
  const { getNetworkType } = appContext;
  ...
  const appContextRef = useRef<any>();
  ...
  if (rxStartsWithBasePath.exec(href) && !myToMainWebView) {
    const relUrl = href.replace(rxStartsWithBasePath, '/');
    linkComp = (
      <Link href={relUrl}>
        <a {...otherProps} />
      </Link>
    );
    // linkComp = <a {...anchorProps} />;
  ...
  } else if ((href.startsWith('/common/tid/login') || href.startsWith('/common/member/slogin/fail')) && isApp()) {
    // TID app sdk 와 web sdk의 차이로 인해 app에서만 replace 처리
    linkComp = <a href={`javascript:location.replace("${href}")`} {...otherProps} />;
  } else linkComp = <a {...anchorProps} />;
