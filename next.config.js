...
if (process.env.DEPLOY_ENV === 'LOCAL') {
  ...
  redirectRules.push({
    source: '/:path((?!v6).*)',
    destination: 'https://m-dev.tworld.co.kr/:path',
    basePath: false,
  });
...
if (process.env.DEPLOY_ENV === 'STG_LOCAL') {
  console.log(`env`, JSON.stringify(process.env, null, 2));
...
if (process.env.DEPLOY_ENV === 'GREEN_LOCAL') {
  console.log(`env`, JSON.stringify(process.env, null, 2));

  redirectRules.push({
    source: '/:path((?!v6).*)',
    destination: 'https://m-prd.tworld.co.kr/:path',
    basePath: false,
  });
